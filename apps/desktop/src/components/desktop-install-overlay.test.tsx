...
describe('DesktopInstallOverlay first-run setup', () => {
  it('shows the remote/local choice without installer progress', async () => {
    ...
    expect(await screen.findByText('Set up Ethel Desktop')).toBeTruthy()
    expect(screen.getByText('Connect to existing Ethel')).toBeTruthy()
    expect(screen.getByText('Install Ethel locally')).toBeTruthy()
    expect(screen.queryByText(/steps complete/i)).toBeNull()
    expect(screen.queryByText(/Fetching installer manifest/i)).toBeNull()
  })

  it('continues local bootstrap only when Install Ethel locally is selected', async () => {
    ...
    fireEvent.click(await screen.findByText('Install Ethel locally'))

    expect(desktop.continueBootstrapLocal).toHaveBeenCalledTimes(1)
    expect(screen.getByText('Set up Ethel Desktop')).toBeTruthy()

    act(() => {
      desktop.emitBootstrapEvent({ type: 'manifest', protocolVersion: 1, stages: [] })
    })

    await waitFor(() => expect(screen.queryByText('Set up Ethel Desktop')).toBeNull())
  ...
  it('surfaces a recoverable error when the local-bootstrap bridge is unavailable', async () => {
    ...
    const install = (await screen.findByText('Install Ethel locally')).closest('button') as HTMLButtonElement
    fireEvent.click(install)

    expect(
      await screen.findByText('Local installation could not start. Restart Ethel Desktop and try again.')
    ).toBeTruthy()
  ...
  it('keeps the local-start error when the first snapshot commits under the click', async () => {
    ...
    const install = (await whenPresent('Install Ethel locally')).closest('button') as HTMLButtonElement
    fireEvent.click(install)
    ...
    expect(screen.queryByText('Local installation could not start. Restart Ethel Desktop and try again.')).toBeTruthy()
  })
  ...
  it('clears a stale local-start error when a repair presents a different root', async () => {
    ...
    fireEvent.click((await screen.findByText('Install Ethel locally')).closest('button') as HTMLButtonElement)
    expect(
      await screen.findByText('Local installation could not start. Restart Ethel Desktop and try again.')
    ).toBeTruthy()
    ...
    expect(screen.queryByText('Local installation could not start. Restart Ethel Desktop and try again.')).toBeNull()
  })
  ...
  it('opens the remote connection form from the first-run choice', async () => {
    ...
    fireEvent.click(await screen.findByText('Connect to existing Ethel'))
  ...
  it('offers remote connection from the unsupported packaged install screen', async () => {
    ...
    expect(await screen.findByText('Ethel needs a one-time install')).toBeTruthy()
    ...
    await waitFor(() => expect(screen.queryByText('Gateway URL')).toBeNull())
    expect(screen.queryByText('Ethel needs a one-time install')).toBeNull()
  })
})
