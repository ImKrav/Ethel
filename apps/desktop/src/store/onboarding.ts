...
function notifyReady(provider: string) {
  notify({ kind: 'success', title: 'Ethel is ready', message: `${provider} connected.` })
}
...
async function completeWithModelConfirm(
  ...
  if (defaults) {
    ...
    try {
      ...
      onFail(error instanceof Error ? error.message : 'Ethel could not save the selected model.')
...
function providerResolutionFailure(reason: null | string) {
  ...
  return detail
    ? `Connected, but Ethel still cannot resolve a usable provider. ${detail}`
    : 'Connected, but Ethel still cannot resolve a usable provider.'
}
...
export async function refreshOnboarding(ctx: OnboardingContext) {
  ...
  if (shouldPreserveConfiguredOnFallback(runtime, state)) {
    ...
    notify({
      ...
      message:
        'Ethel Desktop could not verify the running backend on startup. Some features may be unavailable until the gateway is reachable.'
    })
...
export async function recheckExternalSignin(ctx: OnboardingContext) {
  ...
  await completeWithModelConfirm(ctx, provider.name, [provider.id], reason =>
    setFlow({
      ...
      message:
        reason?.trim() ||
        `Ethel still cannot reach ${provider.name}. Run \`${provider.cli_command}\` in a terminal first.`
    })
  )
}
...
export async function saveOnboardingLocalEndpoint(baseUrl: string, apiKey: string, ctx: OnboardingContext) {
  ...
  try {
    ...
    if (!runtime.ready) {
      ...
      return { ok: false, message: detail || `Saved, but Ethel still cannot reach ${url}.` }
    }
