...
export function enumerationFailureNote(platform: string, env: NodeJS.ProcessEnv, detail?: string): string {
  ...
  if (env.HYPRLAND_INSTANCE_SIGNATURE) {
    return (
      ...
      'Check that `hyprctl clients` works from the same session Ethel is ' +
      'running in.'
    )
  }
  ...
  if (wayland) {
    return (
      ...
      'X11/Xorg session, or run Ethel under XWayland with DISPLAY set.'
    )
  }
...
export async function readWindowBelow(
  ...
  if (process.platform === 'darwin' && !titlesAvailable) {
    result.note =
      ...
      'Screen Recording permission, which Ethel does not request for this.'
  }
