...
function waitForDashboardPort(child, timeoutMs = resolvePortAnnounceTimeoutMs(), describeOutputTail = () => '') {
  return new Promise((resolve, reject) => {
    ...
    function onExit(code, signal) {
      cleanup()
      reject(new Error(`Ethel backend: exited before port announcement (${signal || code})${describeOutputTail()}`))
    }
    ...
    const timer = setTimeout(() => {
      cleanup()
      reject(new Error(`Timed out waiting for Ethel backend port announcement (${timeoutMs}ms)`))
    }, timeoutMs)
