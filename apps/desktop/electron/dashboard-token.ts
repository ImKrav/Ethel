...
async function fetchPublicText(url, options: any = {}) {
  ...
  if (protocol !== 'http:' && protocol !== 'https:') {
    throw new Error(`Unsupported Ethel backend URL protocol: ${protocol}`)
  }
  ...
  const res = await fetch(url, { signal: AbortSignal.timeout(timeoutMs) }).catch(error => {
    if (error.name === 'TimeoutError') {
      throw new Error(`Timed out connecting to Ethel backend after ${timeoutMs}ms`)
    }
 ...
 */
async function adoptServedDashboardToken(baseUrl, spawnToken, { childAlive, label = 'Ethel backend', ...options }) {
