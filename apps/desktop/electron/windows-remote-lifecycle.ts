...
async function probeWindowsRemote(ssh, explicitHermesPath = '') {
  ...
  const script = [
    ...
    'if(-not $hermes){throw "Ethel is not installed on the remote Windows host."}',
    'if($explicit -and $hermes -ne $explicit){throw "The configured Ethel path is not an executable file."}',
    '$python=Join-Path (Split-Path $hermes) "python.exe"',
    'if(-not (Test-Path -LiteralPath $python -PathType Leaf)){throw "The remote Ethel Python runtime was not found."}',
...
async function connectWindowsRemote(deps) {
  ...
  if (!inspection.supported) {
    const error: any = new Error('Update Ethel on the remote Windows host before connecting with Desktop SSH.')
...
function buildWindowsInteractiveCommand(remoteCwd = '') {
  ...
  script.push('$host.UI.RawUI.WindowTitle="Ethel SSH"', 'powershell.exe -NoLogo')
